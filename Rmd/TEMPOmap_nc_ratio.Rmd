...
```{r }
#Cor cluster
res_df <- left_join(res_list$siControl,gene_cl_res,by = "gene")
...
for(i in c("alpha","beta","gamma","nc_slope")){
  ...
  plot_list[[i]] <- ggplot(res_df,aes_string(x = "cluster", y = i, fill = "cluster"))+
    geom_violin() + ylim(quantile(res_df[[i]], probs = c(0,0.99))) + 
...
fviz_nbclust(tmp_df[,c(2:7,11:13)],kmeans,k.max = 15,method = "wss")
set.seed(2021)
...
require(readr)
TEMPO_regression_cc <- read_csv("~/Desktop/At_Broad/Subcellular/TEMPO-regression_cc.csv")
...
tmp_obj %<>% FindClusters(resolution = 0.6)
...
tmp_df <- cl_res_list$sder_cc[,c(1,2:4,5:7,11:13,14,16,18,20)]
...
tmp_df[,c(2:4, 14:16)] %<>% unlist() %>%  scale() %>% matrix(ncol = 3)#a
tmp_df[,c(5:7, 17:19)] %<>% unlist() %>%  scale() %>% matrix(ncol = 3)#beta
tmp_df[,c(11:13, 23:25)] %<>% unlist() %>%  scale() %>% matrix(ncol = 3)#nc
